...
First, we installed Java and Apache Spark using scripts. We opted to use Scala for writing our GraphX jobs, so we also had to install Scala using the following command.

    sudo apt-get install scala

After that, we also installed SBT using the following commands.
...
Don't forget to also source the file.
